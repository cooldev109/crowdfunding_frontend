  ...
  return (
    <AdminLayout>
      <div className="space-y-6 md:space-y-8">
        {/* Header */}
        <div className="relative overflow-hidden rounded-2xl md:rounded-3xl bg-gradient-to-br from-purple-600 via-indigo-600 to-purple-700 p-6 md:p-8 text-white shadow-2xl">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex-1 min-w-0">
              <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm px-3 md:px-4 py-1.5 md:py-2 rounded-full mb-2 md:mb-3 border border-white/30">
                ...
                <span className="text-xs md:text-sm font-semibold">User Management</span>
              </div>
              <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-2">Users Overview</h1>
              <p className="text-white/90 text-sm sm:text-base md:text-lg">
                Manage platform users and permissions
                <span className="hidden sm:inline"> • {filteredUsers.length} total users</span>
              </p>
            </div>
            <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm px-4 md:px-6 py-2 md:py-3 rounded-xl border border-white/30 self-start md:self-center">
              <User size={18} className="md:w-5 md:h-5" />
              <span className="text-xl md:text-2xl font-bold">{users.length}</span>
            </div>
          </div>
        </div>
              ...
              >
                ...
                <CardHeader className="pb-2 pt-3 px-4">
                  <div className="flex items-start gap-2.5">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center text-white font-semibold text-sm flex-shrink-0 shadow-sm group-hover:scale-105 transition-transform duration-300">
                      {user.name.charAt(0).toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <CardTitle className="text-sm font-semibold mb-1.5 truncate group-hover:text-indigo-600 transition-colors">
                        {user.name}
                      </CardTitle>
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        <Badge className={`${getPlanColor(user.planKey)} text-xs font-medium`}>
                          {user.planKey}
                        </Badge>
                      </div>
                    </div>
                ...
                <CardContent className="space-y-2.5 px-4 pb-3">
                  {/* Contact Info */}
                  <div className="p-2 rounded-lg bg-indigo-50/50 border border-indigo-100 space-y-1.5">
                    <div className="flex items-center gap-1.5 text-xs text-gray-700">
                      <Mail size={13} className="text-indigo-600 flex-shrink-0" />
                  ...
                  <div className="flex gap-1.5 pt-2 border-t border-gray-100">
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      className="border border-red-200 hover:bg-red-50 hover:border-red-300 text-red-600 font-medium rounded-lg text-xs h-7 px-2 disabled:opacity-50 disabled:cursor-not-allowed"
