  ...
  return (
    <AdminLayout>
      <div className="space-y-6 md:space-y-8">
        {/* Header */}
        <div className="relative overflow-hidden rounded-2xl md:rounded-3xl bg-gradient-to-br from-green-500 via-emerald-500 to-green-500 p-6 md:p-8 text-white shadow-2xl">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex-1 min-w-0">
              <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm px-3 md:px-4 py-1.5 md:py-2 rounded-full mb-2 md:mb-3 border border-white/30">
                <DollarSign size={16} className="md:w-[18px] md:h-[18px]" />
                <span className="text-xs md:text-sm font-semibold">Investments & Payments</span>
              </div>
              <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-2">Transactions Overview</h1>
              <p className="text-white/90 text-sm sm:text-base md:text-lg">
                Track all platform transactions
                <span className="hidden sm:inline"> • {filteredInvestments.length} total investments</span>
              </p>
            </div>
            <Button className="bg-white text-green-600 hover:bg-white/90 font-bold px-4 md:px-6 py-3 md:py-6 rounded-lg md:rounded-xl shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105 text-sm md:text-base self-start md:self-center">
              <Download size={18} className="md:w-[22px] md:h-[22px] mr-2" />
              Export Report
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
          <Card className="relative border-0 shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 group bg-white overflow-hidden">
            ...
            <CardContent className="pt-4 md:pt-6 relative">
              <div className="flex items-center justify-between gap-2">
                <div className="flex-1 min-w-0">
                  <p className="text-xs md:text-sm text-gray-600 font-semibold uppercase tracking-wide">Total Volume</p>
                  <p className="text-2xl md:text-3xl font-bold text-green-600 mt-1 md:mt-2 truncate">
                    {formatCurrency(totalInvestments)}
                  </p>
                </div>
                <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-gradient-to-br from-green-500 to-emerald-500 flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 flex-shrink-0">
          ...
          <Card className="relative border-0 shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 group bg-white overflow-hidden">
            ...
            <CardContent className="pt-4 md:pt-6 relative">
              <div className="flex items-center justify-between gap-2">
                <div className="flex-1 min-w-0">
                  <p className="text-xs md:text-sm text-gray-600 font-semibold uppercase tracking-wide">Completed</p>
                  <p className="text-2xl md:text-3xl font-bold text-blue-600 mt-1 md:mt-2">{completedCount}</p>
                </div>
                <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 flex-shrink-0">
          ...
          <Card className="relative border-0 shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 group bg-white overflow-hidden">
            ...
            <CardContent className="pt-4 md:pt-6 relative">
              <div className="flex items-center justify-between gap-2">
                <div className="flex-1 min-w-0">
                  <p className="text-xs md:text-sm text-gray-600 font-semibold uppercase tracking-wide">Pending</p>
                  <p className="text-2xl md:text-3xl font-bold text-yellow-600 mt-1 md:mt-2">{pendingCount}</p>
                </div>
                <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-gradient-to-br from-yellow-500 to-orange-500 flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 flex-shrink-0">
          ...
          <CardContent className="pt-4 md:pt-6">
            <div className="flex flex-col md:flex-row gap-3 md:gap-4">
              <div className="flex-1 relative">
                <Search className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2 text-green-400" size={18} />
                <Input
                  placeholder="Search by user or project..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="pl-10 md:pl-12 h-10 md:h-12 border-2 border-green-100 focus:border-green-300 rounded-xl text-sm md:text-base"
                />
              </div>
